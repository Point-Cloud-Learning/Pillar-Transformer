...
class Pillar_Encoder(nn.Module):
    def __init__(self, embed_dim=768, interval=0.2, layer_setting=([6, 32], [32, 64], [64, 128]), aggregation_setting=([256, 768],), norm_layer=None,
        ...
        for in_feature, out_feature in layer_setting:
            ...
            layer_nor = nn.LayerNorm(out_feature)
            relu = nn.ReLU()
            self.sequential1.append(linear)
            self.sequential1.append(layer_nor)
            self.sequential1.append(relu)
        for in_feature, out_feature in aggregation_setting:
            linear = nn.Linear(in_features=in_feature, out_features=out_feature, bias=False)
            layer_nor = nn.LayerNorm(out_feature)
            relu = nn.ReLU()
            self.sequential2.append(linear)
            self.sequential2.append(layer_nor)
            self.sequential2.append(relu)
    ...
    def pillar_allocation(self, one_data: torch.tensor):
        ...
        for i in range(num_of_point):
            indices = tuple((torch.div((one_data[i][1:] + 1.0).clamp(0, 1.999), self.interval, rounding_mode="trunc")).numpy().tolist())
    ...
    def forward(self, x: torch.tensor):
        ...
        batch_sparse_sequences = batch_sparse_matrix.flatten(2).permute(0, 2, 1)
